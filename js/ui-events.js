...
function navigateJobs(direction) {
  ...
  if (jobCards[nextIndex]) {
    ...
    jobCards[nextIndex].scrollIntoView({
      behavior: "smooth",
      block: "nearest",
    });
  }
...
function handleJobCardClick(event) {
  ...
  if (!currentAppliedState[currentThreadId])
    currentAppliedState[currentThreadId] = {};
  if (!currentNotesState[currentThreadId])
    currentNotesState[currentThreadId] = {};
  if (!currentFavoritesState[currentThreadId])
    currentFavoritesState[currentThreadId] = {};
  if (!currentHiddenState[currentThreadId])
    currentHiddenState[currentThreadId] = {};
