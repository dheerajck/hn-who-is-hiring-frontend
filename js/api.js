...
export async function fetchLatestThreadListFromApi(
  ...
  if (USE_MOCK_DATA) {
    ...
    return new Promise((resolve) => {
      setTimeout(
        ...
        isBackground ? 2000 : 500
      ); // Simulate network delay for background
    });
  }
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch thread list for category "${query}": ${res.statusText}`
    );
  }
