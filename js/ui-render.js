...
import { allThreads, currentCategory, currentThreadId, allComments, favorites, notes, applied, hidden, activeToastHideTimerId, selectedYear, setSelectedYear, setCurrentCategory, setAllComments, setActiveToastHideTimerId } from './state.js';
...
export function renderCategorySwitcher() {
    ...
    for (const category in CATEGORY_API_MAP) {
        ...
        button.id = `category${category.charAt(0).toUpperCase() + category.slice(1)}`;
        ...
        yearBtn.addEventListener('click', () => {
            if (selectedYear !== year) {
                ...
                const monthsForYear = Array.from(threadsByYearMonth.get(selectedYear).keys())
                                        .sort((a, b) => MONTH_NAMES.indexOf(MONTH_NAMES.find(m => m.toLowerCase() === b.toLowerCase())) - MONTH_NAMES.indexOf(MONTH_NAMES.find(m => m.toLowerCase() === a.toLowerCase())));
...
export function renderJobs(commentsToRender) {
    ...
    const showFavs = document.getElementById('showFavorites')?.classList.contains(highlightClass) || false;
    const showApplied = document.getElementById('showApplied')?.classList.contains(highlightClass) || false;
    const showNotes = document.getElementById('showNotes')?.classList.contains(highlightClass) || false;
    const hideAppliedActive = document.getElementById('hideAppliedBtn')?.classList.contains(highlightClass) || false;
    const showHiddenActive = document.getElementById('showHidden')?.classList.contains(highlightClass) || false;
    ...
    filteredComments.forEach(c => {
        ...
        if (c.created_at) {
            ...
            const formattedDate = d.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true });
            const now = new Date();
            const diffMs = now - d;
        ...
        let jobTitle = rawTitleLine.includes('|') ? rawTitleLine.split('|')[0].trim() : rawTitleLine;
            ...
            <div class="job-header">
                <div class="job-header-top" style="width:100%;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;margin-bottom:0.5rem;">
                    <div class="job-header-status" style="display:flex;align-items:center;gap:0.5rem;">
                        ${appliedStatus ?
                            `<span class="badge" style="font-weight:bold;letter-spacing:0.5px;">Applied</span>
                            <div class="meta"><i class="far fa-calendar"></i> ${new Date(appliedStatus).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit', hour12: true })}</div>`
                            : ''}
                    </div>
                    <div style="font-weight:normal; color:#999; font-size:0.9em;">${postedTime ? `${postedTime}` : ''}</div>
                </div>
                <div style="display:flex;align-items:flex-start;margin-bottom:0.75rem;gap:0.75rem;">
                    <button class="action-btn star-btn${isFav ? '' : ' inactive'}" data-action="star" title="Add to Favorite" aria-label="Star job"><i class="fas fa-star"></i></button>
